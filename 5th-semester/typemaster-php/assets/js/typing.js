...
class TypingTest {
    constructor() {
        ...
        this.isLoadingNewTest = false;
        this.boundGlobalShortcutHandler = null;
    }
    ...
    setupEventListeners() {
        ...
        if (!this.boundGlobalShortcutHandler) {
            this.boundGlobalShortcutHandler = (e) => this.handleGlobalShortcuts(e);
            document.addEventListener('keydown', this.boundGlobalShortcutHandler);
        }
    ...
    handleGlobalShortcuts(e) {
        ...
        if (!this.isCompleted || this.isActive || this.isLoadingNewTest) return;
        if (!this.resultsModal || this.resultsModal.style.display === "none") return;
    ...
    async newTest() {
        if (this.isLoadingNewTest) return;
        this.isLoadingNewTest = true;
        try {
            await this.loadParagraph();
            this.restart();
        } finally {
